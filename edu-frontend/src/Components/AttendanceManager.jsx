                      ...
                      <td className="px-5 py-3.5">
                        <div className="flex justify-center items-center gap-1.5">
                          <button 
                            ...
                            className={`px-3 py-1 rounded text-xs font-semibold transition cursor-pointer border ${status === 'PRESENT' ? 'bg-emerald-600 text-white border-emerald-600 shadow-xs' : 'bg-white text-gray-700 border-gray-300 hover:bg-emerald-50 hover:text-emerald-700'}`}>
                            Có mặt
                          </button>
                          <button 
                            type="button"
                            onClick={() => handleStatusChange(st.studentId, 'LATE')}
                            className={`px-3 py-1 rounded text-xs font-semibold transition cursor-pointer border ${status === 'LATE' ? 'bg-amber-500 text-white border-amber-500 shadow-xs' : 'bg-white text-gray-700 border-gray-300 hover:bg-amber-50 hover:text-amber-700'}`}>
                            Đi muộn
                          </button>
                          <button 
                            type="button"
                            onClick={() => handleStatusChange(st.studentId, 'ABSENT')}
                            className={`px-3 py-1 rounded text-xs font-semibold transition cursor-pointer border ${status === 'ABSENT' ? 'bg-red-600 text-white border-red-600 shadow-xs' : 'bg-white text-gray-700 border-gray-300 hover:bg-red-50 hover:text-red-700'}`}>
                            Vắng mặt
                          </button>
                        </div>
                      </td>
